...
gtf_file <- file.path(ref_dir, "gencode.v43.annotation.gtf.gz")
# Use gtf_file in your R code
...
#Set working directory to a folder containing the training, reference, and query data
#File can be downloaded from https://github.com/CahanLab/PACNet/
...
expTrain<-utils_loadObject("/Users/kjakubiak/Downloads/hsTFs.rda")
#stTrain <- utils_loadObject("Hs_stTrain_Jun-20-2017.rda")
stTrain<-utils_loadObject("/Users/kjakubiak/Downloads/mmTFs.rda")
...
neuronRefExpDat <- utils_loadObject("neuron_engineeredRef_normalized_expDat_all.rda")
neuronRefSampTab <- utils_loadObject("neuron_engineeredRef_sampTab_all.rda")
...
study_name <- "neuron_example"

# Identify intersecting genes
iGenes <- intersect(rownames(expTrain), rownames(neuronRefExpDat))
...
classMatrixneuronRef <- broadClass_predict(cnProc = cnProc, expDat = neuronRefExpDat, nrand = 10) 
grp_names1 <- c(as.character(neuronRefSampTab$description1), rep("random", 10))
names(grp_names1) <- c(as.character(rownames(neuronRefSampTab)), paste0("rand_", c(1:10)))
# Re-order classMatrixQuery to match order of rows in querySampTab
classMatrixneuronRef <- classMatrixneuronRef[,names(grp_names1)]

png(file="example_outputs/heatmapneuronRef.png", height=12, width=9, units="in", res=300)
heatmapRef(classMatrixneuronRef, neuronRefSampTab) # This function can be found in pacnet_utils.R
dev.off()

# Alternatively, for an interactive plotly version:
heatmapPlotlyRef(classMatrixneuronRef, neuronRefSampTab)
...
grnAll <- utils_loadObject("neuron_grnAll.rda")
trainNormParam <- utils_loadObject("neuron_trainNormParam.rda")
...
target_cell_type <- "neuron" # CHANGE FOR SPECIFIC CONTEXT
