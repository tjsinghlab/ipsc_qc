...
sig_matrix<-read.table("formative_v_primed_gene_expression.txt",header=T,sep="\t",row.names=1,check.names=F)
rownames(sig_matrix)<-sig_matrix$Gene
sig_matrix$Gene=NULL
bulk_ipsc<-readRDS("ipsc_858samples_expected_counts_with_logenorm_peer_factors_cellcycle_and_vst_matrix")

results <- CIBERSORT(sig_matrix,as.matrix(bulk_ipsc@assays$AdjExpr@counts))

res<-data.frame(results)
res$Sample<-rownames(res)

metadata<-bulk_ipsc@meta.data
metadata<-full_join(metadata, res, "Sample")
